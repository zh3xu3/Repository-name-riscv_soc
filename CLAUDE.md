...
**阻塞级问题：**
1. B1: CSR 写未被 `irq_trap` 抑制 → 添加 `&& !irq_trap`
2. B4: `mtvec` 模式位未 mask → `{mtvec[31:2], 2'b00}`
3. B8: stall 期间 BHT 被重复更新 → `update_en && !stall`
4. B9: CSRRS/CSRRC 对非 mstatus 寄存器不生效 → 添加 read-modify-write 支持

### 新增功能

| 模块 | 地址 | 功能 |
|------|------|------|
| DMA | 0x0000_8000 | 内存到内存/外设数据传输 |
| I-Cache | 0x0000_9000 | 2KB 直接映射指令 Cache |
| I2C | 0x0000_A000 | I2C Master 控制器 |

### 地址空间更新

| 地址范围 | 外设 |
|----------|------|
| 0x0000_0000-0FFF | 指令存储器 (4KB) |
| 0x0000_1000-1FFF | 数据存储器 (4KB) |
| 0x0000_2000-200F | UART |
| 0x0000_3000-300F | GPIO |
| 0x0000_4000-400F | WDT |
| 0x0000_5000-500F | SPI |
| 0x0000_6000-601F | PWM |
| 0x0000_7000-703F | PLIC |
| 0x0000_8000-801F | DMA |
| 0x0000_9000-900F | I-Cache |
| 0x0000_A000-A01F | I2C |
